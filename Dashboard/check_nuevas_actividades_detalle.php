...
// Marcar como leídas las actividades encontradas
...
if ($stmt) {
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) $meritos_semana = $row['total'];
    $stmt->close();
}
...
if ($stmt) {
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) $demeritos_semana = $row['total'];
    $stmt->close();
}

// Contar notificaciones nuevas (leido = 0)
$nuevas_count = 0;
...
if ($stmt) {
    ...
    $stmt->execute();
    $result = $stmt->get_result();
    if ($row = $result->fetch_assoc()) $nuevas_count = $row['total'];
    $stmt->close();
}
...
echo json_encode([
    ...
    'stats' => [
        ...
        'balance_semana' => (int)($meritos_semana - $demeritos_semana)
    ],
